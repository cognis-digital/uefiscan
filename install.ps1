# Comprehensive installer for cognis-digital/uefiscan (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# uefiscan is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "uefiscan"
$Url  = "git+https://github.com/cognis-digital/uefiscan.git"
$Git  = "https://github.com/cognis-digital/uefiscan.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: uefiscan"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: uefiscan"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: uefiscan"; exit 0 }
}
