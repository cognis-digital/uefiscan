...
[![PyPI](https://img.shields.io/pypi/v/cognis-uefiscan.svg)](https://pypi.org/project/cognis-uefiscan/)
[![CI](https://github.com/cognis-digital/uefiscan/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/uefiscan/actions)
...
UEFISCAN exists for one job — audit uefi firmware dumps for missing secure boot keys, unsigned modules, s3 boot-script vulns, and known smm threats. — and does it without a SaaS bill or heavyweight setup.
...
```bash
uefiscan --version
uefiscan scan .                      # scan the current project
uefiscan scan . --format json
uefiscan scan . --fail-on high       # non-zero exit for CI gates
uefiscan mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **CHIPSEC + UEFITool**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`uefiscan` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
