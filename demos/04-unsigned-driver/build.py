...
Run from the repo root:  python demos/04-unsigned-driver/build.py
"""
import os, sys
...
def build():
    return fw.assemble(fw.firmware_volume(), fw.secureboot_vars(["PK","KEK","db","dbx"]), fw.pe_module(signed=True), fw.pe_module(signed=False), fw.pe_module(signed=True))
