#!/usr/bin/env sh
# Comprehensive installer for cognis-digital/uefiscan (Linux / macOS).
...
GITURL="https://github.com/cognis-digital/uefiscan.git"
...
if have pipx; then
  ...
  pipx install "$URL" && { say "Done. Run: uefiscan"; exit 0; }
fi
if have uv; then
  say "Installing with uv..."
  uv tool install "$URL" && { say "Done. Run: uefiscan"; exit 0; }
fi
if have pip3 || have pip; then
  PIP="$(command -v pip3 || command -v pip)"
  say "Installing with pip (user site)..."
  "$PIP" install --user "$URL" && { say "Done. Run: uefiscan"; exit 0; }
fi
